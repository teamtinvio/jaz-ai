...
export async function bulkUpsertCurrencyRates(
  client: JazClient,
  rates: Array<{
    sourceCurrencyCode: string;
    rate: number;
    rateDirection: 'FUNCTIONAL_TO_SOURCE' | 'SOURCE_TO_FUNCTIONAL';
    rateApplicableFrom: string;
    rateApplicableTo?: string;
  }>,
): Promise<{ data: { resourceId: null; resourceIds: string[] } }> {
  return client.post('/api/v1/organization/currencies/rates/bulk-upsert', { rates });
}
