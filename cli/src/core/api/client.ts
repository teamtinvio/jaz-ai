...
/** Emitted after each API request (success or error). */
...
export interface ClientOptions {
  ...
  timeout?: number;
  maxRetries?: number;
  /** Callback after each request — used by daemon for API metrics. */
...
export class JazClient {
  ...
  private async request<T>(
    ...
    // Circuit breaker: fail fast if this endpoint has too many consecutive failures
    checkCircuit(path);
    ...
    for (let attempt = 0; attempt <= this.maxRetries; attempt++) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          this.onRequest?.({
            method, path, statusCode: response.status,
            durationMs: Date.now() - attemptStart, attempt, retried: false,
            error: formatErrorBody(body),
          });
          // Record server errors for circuit breaker (not client errors like 4xx)
          if (response.status >= 500) recordFailure(path);
        ...
        this.onRequest?.({
          ...
          durationMs: Date.now() - attemptStart, attempt,
          retried: attempt < this.maxRetries, error: lastError.message,
        });
  ...
  async listAll<T>(path: string, pageSize = 200): Promise<T[]> {
    ...
    while (hasMore) {
      const result = await this.list<T>(path, { limit: pageSize, offset: page });
