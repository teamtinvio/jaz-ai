...
export interface LineItem {
  ...
  /** GET responses use this field name instead of accountResourceId */
...
export interface BankRecord {
  ...
  bankStatementEntrySource?: string;
  externalTransactionId?: string;
  archiveNotes?: string;
  reconciledBy?: { userResourceId: string; firstName: string; lastName: string };
  sourceMetadata?: { type: string; correlationId: string };
  metadata?: Array<{ columnName: string; columnValue: string }>;
}

export interface AddBankRecordEntry {
  amount: number;
  transactionDate: string;
  description?: string;
  payerOrPayee?: string;
  reference?: string;
  metadata?: Array<{ index: number; name: string; value: string }>;
}
...
export interface CashflowTransaction {
  resourceId: string;
  parentEntityResourceId: string; // journal header ID — use for DELETE (/cash-entries/:id)
...
export type ExportType =
  ...
  | 'vat-ledger' | 'equity-movement'
  | 'bank-balance-summary' | 'bank-reconciliation-summary' | 'bank-reconciliation-details'
  | 'fixed-assets-summary' | 'fixed-assets-recon-summary'
  | 'contact-sales-summary' | 'contact-purchase-summary'
  | 'journals-summary';
...
export interface BankRule {
  ...
  name: string;
  actionType: string;
  appliesToReconciliationAccount: {
    code: string | null;
    currencyCode: string;
    name: string;
  };
...
export interface FixedAsset {
  ...
  netBookValueAmount: number;
  registrationType: string;
  disposalValueDate: number | null;
  assetDisposalGainLossAmount: number;
  netBookAtDisposalAmount: number;
...
export interface Subscription {
  ...
  businessTransactionType: string;
  contactResourceId: string | null;
  contact: { name: string; resourceId: string } | null;
...
export interface ContactGroup {
  resourceId: string;
  name: string;
  associatedContacts: Array<{ name: string; resourceId: string }>;
}
...
export interface InventoryBalance {
  itemResourceId: string;
  latestAverageCostAmount: number;
  baseQty: number;
  baseUnit: string;
}

// ── Withholding Tax Code Types ──────────────────────────────────
