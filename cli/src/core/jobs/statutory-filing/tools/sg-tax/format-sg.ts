...
import { muted, success, danger, warning, highlight } from '../../../../../commands/ui/theme.js';
...
const dline = (w: number): string => muted('═'.repeat(w));
...
export function printFormCsResult(result: SgFormCsResult): void {
  ...
  console.log();
  console.log(highlight(`SINGAPORE CORPORATE INCOME TAX COMPUTATION — YA ${result.ya}`));
  console.log(line(W));
  console.log(`  Form type:      ${highlight(result.formType)}${result.eligible ? success(' ✓ Eligible') : danger(' ✗ Not eligible')}`);
  ...
  console.log();
  console.log(highlight('TAX COMPUTATION SCHEDULE'));
  console.log(line(W));
  ...
  if (result.unabsorbedLosses > 0 || result.unabsorbedCapitalAllowances > 0 || result.unabsorbedDonations > 0) {
    console.log();
    console.log(highlight('CARRY-FORWARDS TO NEXT YA'));
    console.log(line(W));
  ...
  console.log();
  console.log(highlight(`FORM ${result.formType} FIELD MAPPING`));
  console.log(line(W));
...
function printScheduleLine(entry: TaxScheduleLine, ccy: string, _w: number): void {
  ...
  const ref = entry.reference ? muted(` (${entry.reference})`) : '';
  ...
  if (indent === 0) {
    ...
    console.log(muted(`${prefix}${label.padEnd(43 - indent * 2)}${ccy} ${fmtR(entry.amount)}${ref}`));
  }
}
...
export function printCaResult(result: SgCapitalAllowanceResult): void {
  ...
  console.log();
  console.log(highlight(`CAPITAL ALLOWANCE SCHEDULE — YA ${result.ya}`));
  console.log(line(W));
  ...
  console.log(highlight(`  Total CA available:          ${ccy} ${fmt(result.totalAvailable)}`));

  if (result.lowValueCapped) {
    console.log();
    console.log(warning(`  Note: Low-value asset claims capped at ${ccy} 30,000.00 per YA`));
    console.log(warning(`  (${result.lowValueCount} low-value assets, total ${ccy} ${fmt(result.lowValueTotal)} claimed)`));
  }
