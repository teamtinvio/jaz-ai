...
import { accent, muted, success, warning, highlight } from '../../../../../commands/ui/theme.js';
...
export function printBankFileResult(result: BankFileResult, savedTo?: string): void {
  ...
  console.log();
  console.log(highlight(`BANK FILE GENERATED — ${result.summary.formatDescription}`));
  console.log(line(W));
  if (savedTo) {
    console.log(`  Saved to:     ${success(savedTo)}`);
  } else {
    console.log(`  Filename:     ${accent(result.filename)}`);
  }
