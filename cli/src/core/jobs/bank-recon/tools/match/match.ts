...
import type { SubsetCandidate, SubsetResult } from './match-utils.js';
...
export function matchBankRecords(input: BankMatchInput): BankMatchResult {
  ...
  for (const ri of [...recordPool]) {
    ...
    const candidates: { ti: number; cents: number; contactNorm: string }[] = [];
    for (const ti of txnPool) {
      const t = txns[ti];
      if ((r.cents > 0) !== (t.cents > 0)) continue;
    ...
    let bestSubset: SubsetResult | null = null;
    ...
    for (const groupCandidates of contactCandidateGroups.values()) {
      ...
      const subCandidates: SubsetCandidate[] = groupCandidates.map((c, i) => ({
        index: i,
...
function groupByContact<T extends { contactNorm: string }>(candidates: T[]): Map<string, T[]> {
