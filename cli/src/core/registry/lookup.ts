...
/**
 * Register TUI-only tools. Called once from agent launch before the loop starts.
 * This avoids require() in ESM and keeps the registry synchronous.
 */
...
/** Get all tool definitions (includes TUI tools if registered). */
export function getAllTools(): ToolDefinition[] {
  return _tuiTools.length > 0 ? [...TOOL_DEFINITIONS, ..._tuiTools] : TOOL_DEFINITIONS;
}
