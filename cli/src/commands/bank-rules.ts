  ...
  cmd
    ...
    .action(apiAction(async (client, opts) => {
      ...
      if (!body) {
        ...
        if (opts.config) {
          try { body.configuration = JSON.parse(opts.config); } catch { console.error(chalk.red('Invalid --config JSON')); process.exit(1); }
        }
      }
  ...
  cmd
    ...
    .action((resourceId: string, opts) => apiAction(async (client) => {
      ...
      if (body) {
        ...
        if (opts.config) {
          try { data.configuration = JSON.parse(opts.config); } catch { console.error(chalk.red('Invalid --config JSON')); process.exit(1); }
        }
      }
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
