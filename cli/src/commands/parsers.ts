...
import { danger, muted } from './ui/theme.js';
...
export function parsePositiveInt(raw: string): number {
  ...
  if (!Number.isFinite(n) || n <= 0) {
    console.error(danger(`Error: invalid positive integer: ${raw}`));
    process.exit(1);
  }
  return n;
}
...
export function parseNonNegativeInt(raw: string): number {
  const n = Number.parseInt(raw, 10);
  if (!Number.isFinite(n) || n < 0) {
    console.error(danger(`Error: invalid non-negative integer: ${raw}`));
    process.exit(1);
  }
  return n;
}
...
export function parseMoney(raw: string): number {
  if (!/^-?\d+(\.\d{1,2})?$/.test(raw)) {
    console.error(danger(`Error: invalid amount: ${raw} (max 2 decimal places)`));
    process.exit(1);
  }
...
export function parseRate(raw: string): number {
  const n = Number(raw);
  if (!Number.isFinite(n) || n <= 0) {
    console.error(danger(`Error: invalid exchange rate: ${raw}`));
    process.exit(1);
  }
  return n;
}
...
export function parseLineItems(raw: string): Array<Record<string, unknown>> {
  try {
    ...
    console.error(danger('Error: --lines must be valid JSON array'));
    console.error(muted('Example: --lines \'[{"name":"Service","quantity":1,"unitPrice":100,"accountResourceId":"..."}]\''));
    console.error(muted('Tip: pipe the full request body via stdin, or use --input <file>'));
    process.exit(1);
  }
}
...
export function parseJournalEntries(raw: string): Array<Record<string, unknown>> {
  try {
    ...
    console.error(danger('Error: --entries must be valid JSON array'));
    console.error(muted('Example: --entries \'[{"accountResourceId":"...","amount":100,"type":"DEBIT"},...]\''));
    console.error(muted('Tip: pipe the full request body via stdin, or use --input <file>'));
    process.exit(1);
  }
}

export function parseCustomFields(raw: string): Array<{ customFieldName: string; actualValue: string }> {
  try {
    const parsed = JSON.parse(raw);
    if (!Array.isArray(parsed)) throw new Error('Must be a JSON array');
    return parsed;
  } catch {
    console.error(danger('Error: --custom-fields must be valid JSON array'));
    console.error(muted('Example: --custom-fields \'[{"customFieldName":"PO Number","actualValue":"PO-123"}]\''));
    process.exit(1);
  }
}
...
export function readBodyInput(opts: { input?: string }): Record<string, unknown> | null {
  if (opts.input) {
    ...
    try {
      ...
      console.error(danger(`Error: cannot read file: ${opts.input}`));
      process.exit(1);
    }
    try {
      const parsed = JSON.parse(raw);
      if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
        console.error(danger('Error: --input file must contain a JSON object'));
        process.exit(1);
      }
      return parsed as Record<string, unknown>;
    } catch {
      console.error(danger('Error: --input file contains invalid JSON'));
      process.exit(1);
    }
  }
  ...
  if (!process.stdin.isTTY) {
    try {
      ...
      if (raw) {
        const parsed = JSON.parse(raw);
        if (typeof parsed !== 'object' || parsed === null || Array.isArray(parsed)) {
          console.error(danger('Error: stdin must contain a JSON object'));
          process.exit(1);
        }
        return parsed as Record<string, unknown>;
      }
      ...
      if ((err as Error).message?.includes('JSON')) {
        console.error(danger('Error: stdin contains invalid JSON'));
        process.exit(1);
      }
    }
  }
...
export function requireFields(
  ...
  if (missing.length > 0) {
    ...
    console.error(danger(`Error: missing required option(s): ${flags}`));
    console.error(muted('Provide flags, or pipe JSON via stdin, or use --input <file>'));
    process.exit(1);
  }
}
