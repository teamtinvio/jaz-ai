...
export function parseCustomFields(raw: string): Array<{ customFieldName: string; actualValue: string }> {
  try {
    ...
    console.error(chalk.red('Error: --custom-fields must be valid JSON array'));
    console.error(chalk.dim('Example: --custom-fields \'[{"customFieldName":"PO Number","actualValue":"PO-123"}]\''));
    process.exit(1);
  }
}
