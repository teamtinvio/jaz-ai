...
export function registerCustomFieldsCommand(program: Command): void {
  ...
  cmd
    ...
    .action((resourceId: string, opts) => apiAction(async (client) => {
      ...
      if (f.description) console.log(highlight('Description:'), f.description);
      if (f.defaultValue) console.log(highlight('Default:'), f.defaultValue);
      if (f.listOptions?.length) console.log(highlight('Options:'), f.listOptions.join(', '));
      ...
      if (applies.length) console.log(highlight('Applies to:'), applies.join(', '));
    })(opts));
