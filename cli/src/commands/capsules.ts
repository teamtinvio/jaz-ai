...
const CAPSULES_COLUMNS: TableColumn[] = [
...
export function registerCapsulesCommand(program: Command): void {
  ...
  capsules
    ...
    .action(apiAction(async (client, opts) => {
      ...
      outputList(result as any, CAPSULES_COLUMNS, opts as OutputOpts, 'Capsules');  // eslint-disable-line @typescript-eslint/no-explicit-any
    }));
  ...
  capsules
    ...
    .action((resourceId: string, opts) => apiAction(async (client) => {
      ...
      if (opts.json) {
        ...
        if (c.description) console.log(highlight('Description:'), c.description);
        console.log(highlight('ID:'), c.resourceId);
        if (c.type) console.log(highlight('Type:'), c.type);
        if (c.capsuleType?.resourceId) console.log(highlight('Type ID:'), c.capsuleType.resourceId);
      }
    })(opts));
