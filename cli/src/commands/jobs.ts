...
async function fetchOutstandingForBankFile(
  ...
  process.stderr.write(
    chalk.bold(`  Found ${outstanding.supplierCount} supplier${outstanding.supplierCount !== 1 ? 's' : ''}`) +
  ...
  const originatorAnswers = await prompts([
    ...
    },
    {
      ...
      validate: (v: string) => /^\d{4}-\d{2}-\d{2}$/.test(v) || 'Format: YYYY-MM-DD',
    },
  ], { onCancel: () => process.exit(0) });
  ...
  paymentRun
    ...
    .action(apiAction(async (client, opts) => {
      ...
      if (fetched.truncated && !opts.json) {
        process.stderr.write(
          chalk.yellow(`  Warning: ${res.totalElements.toLocaleString()} bills found, showing ${res.data.length.toLocaleString()} (API pagination bug — see api-dx-issues.md #9)\n`),
        );
      }
