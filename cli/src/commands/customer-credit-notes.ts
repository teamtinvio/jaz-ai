...
export function registerCustomerCreditNotesCommand(program: Command): void {
  ...
  ccn
    ...
    .action(apiAction(async (client, opts) => {
      ...
      outputList(result as any, CCN_COLUMNS, opts as OutputOpts, 'Customer Credit Notes');  // eslint-disable-line @typescript-eslint/no-explicit-any
    }));
  ...
  // ── clio customer-credit-notes download ──────────────────────
  ccn
    .command('download <resourceId>')
    .description('Download a customer credit note as PDF (returns URL)')
    .option('--api-key <key>', 'API key (overrides stored/env)')
    .option('--json', 'Output as JSON')
    .action((resourceId: string, opts) => apiAction(async (client) => {
      const res = await downloadCustomerCreditNotePdf(client, resourceId);

      if (opts.json) {
        console.log(JSON.stringify(res.data, null, 2));
      } else {
        console.log(chalk.green('PDF download ready:'));
        console.log(chalk.dim(res.data.fileUrl));
      }
    })(opts));
