...
const BILL_COLUMNS: TableColumn[] = [
...
export function registerBillsCommand(program: Command): void {
  ...
  bills
    ...
    .action(apiAction(async (client, opts) => {
      ...
      outputList(result as any, BILL_COLUMNS, opts as OutputOpts, 'Bills');  // eslint-disable-line @typescript-eslint/no-explicit-any
    }));
  ...
  bills
    ...
    .action((resourceId: string, opts) => apiAction(async (client) => {
      ...
      if (opts.json) {
        ...
        if (b.contactName) console.log(highlight('Contact:'), b.contactName);
        if (b.totalAmount !== undefined) console.log(highlight('Total:'), b.totalAmount.toFixed(2));
        if (b.amountDue !== undefined) console.log(highlight('Amount Due:'), b.amountDue.toFixed(2));
        ...
        if (b.notes) console.log(highlight('Notes:'), b.notes);
      }
    })(opts));
