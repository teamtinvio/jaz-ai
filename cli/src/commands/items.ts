...
export function registerItemsCommand(program: Command): void {
  ...
  items
    ...
    .action((resourceId: string, opts) => apiAction(async (client) => {
      ...
      if (opts.json) {
        ...
        if (types) console.log(highlight('Applies to:'), types);
      }
    })(opts));
  ...
  items
    .command('bulk-upsert')
    .description('Create or update items in bulk (max 500). Provide resourceId per item to update, omit to create.')
    ...
    .action(apiAction(async (client, opts) => {
      const body = readBodyInput(opts);
      if (!body) {
        console.error('Error: --input <file> is required (JSON with { "items": [...] })');
        process.exit(1);
      }
      const items = (body as Record<string, unknown>).items ?? body;
      if (!Array.isArray(items)) {
        console.error('Error: expected { "items": [...] } or a JSON array');
        process.exit(1);
      }
      if (items.length > 500) {
        console.error(`Error: max 500 items per call (got ${items.length}). Split into batches.`);
        process.exit(1);
      }
      const res = await bulkUpsertItems(client, items);
      if (opts.json) {
        console.log(JSON.stringify(res.data, null, 2));
      } else {
        const ids = res.data.resourceIds ?? [];
        console.log(success(`${ids.length} item(s) upserted.`));
