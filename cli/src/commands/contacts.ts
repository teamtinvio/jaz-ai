...
const CONTACTS_COLUMNS: TableColumn[] = [
  { key: 'resourceId', header: 'ID', format: formatId },
  { key: 'billingName', header: 'Name' },
  { key: 'customer', header: 'Customer', format: (v) => v ? 'Yes' : '-' },
  { key: 'supplier', header: 'Supplier', format: (v) => v ? 'Yes' : '-' },
];
...
export function registerContactsCommand(program: Command): void {
  ...
  contacts
    ...
    .action(apiAction(async (client, opts) => {
      ...
      outputList(result as any, CONTACTS_COLUMNS, opts as OutputOpts, 'Contacts');  // eslint-disable-line @typescript-eslint/no-explicit-any
    }));
  ...
  contacts
    ...
    .action((resourceId: string, opts) => apiAction(async (client) => {
      ...
      if (opts.json) {
        ...
        if (roles) console.log(highlight('Roles:'), roles);
        if (c.status) console.log(highlight('Status:'), c.status);
      }
    })(opts));
  ...
  contacts
    ...
    .action(apiAction(async (client, opts) => {
      ...
      // Guard: check for existing contact before creating
      const contactName = (body as Record<string, unknown>)?.name as string ?? opts.name;
      if (contactName) {
        const existing = await findExistingContact(client, contactName);
        if (existing) {
          if (opts.json) {
