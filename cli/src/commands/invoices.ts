  ...
  invoices
    ...
    .action((resourceId: string, opts) => apiAction(async (client) => {
      ...
      if (opts.json) {
        ...
        if (inv.contactName) console.log(highlight('Contact:'), inv.contactName);
        if (inv.totalAmount !== undefined) console.log(highlight('Total:'), inv.totalAmount.toFixed(2));
        if (inv.amountDue !== undefined) console.log(highlight('Amount Due:'), inv.amountDue.toFixed(2));
        ...
        if (inv.notes) console.log(highlight('Notes:'), inv.notes);
      }
    })(opts));
