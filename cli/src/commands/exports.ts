...
export function registerExportsCommand(program: Command): void {
  ...
  cmd
    ...
    .action(apiAction(async (client, opts) => {
      ...
      const exportType = opts.type as ExportType;
      ...
      const res = await downloadExport(client, exportType, params);
      ...
      if (opts.json) {
        ...
        if (d.fileName) console.log(highlight('File:'), d.fileName);
        if (d.fileUrl) console.log(highlight('URL:'), d.fileUrl);
      }
    }));
}
