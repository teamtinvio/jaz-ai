...
import {
  ...
  searchSupplierCreditNotes,
  createSupplierCreditNote,
  updateSupplierCreditNote,
  deleteSupplierCreditNote,
  createSupplierCreditNoteRefund,
  listSupplierCreditNoteRefunds,
  finalizeSupplierCreditNote,
} from '../core/api/supplier-cn.js';
...
export function registerSupplierCreditNotesCommand(program: Command): void {
  const scn = program
    .command('supplier-credit-notes')
    .description('Manage supplier credit notes');

  // ── clio supplier-credit-notes list ───────────────────────────
  scn
    .command('list')
    ...
    .action(apiAction(async (client, opts) => {
      const result = await paginatedFetch(
        opts,
        (p) => listSupplierCreditNotes(client, p),
        { label: 'Fetching supplier credit notes' },
      );

      outputList(result as any, SCN_COLUMNS, opts as OutputOpts, 'Supplier Credit Notes');  // eslint-disable-line @typescript-eslint/no-explicit-any
    }));

  // ── clio supplier-credit-notes get ────────────────────────────
  scn
    .command('get <resourceId>')
    .description('Get a supplier credit note by resourceId')
    .option('--api-key <key>', 'API key (overrides stored/env)')
    .option('--json', 'Output as JSON')
    .action((resourceId: string, opts) => apiAction(async (client) => {
      const res = await getSupplierCreditNote(client, resourceId);
      const cn = res.data;
      ...
      if (opts.json) {
        ...
        console.log(chalk.bold('Date:'), normalizeDate(cn.valueDate));
  ...
  scn
    .command('search')
    .description('Search supplier credit notes')
    ...
    .action(apiAction(async (client, opts) => {
      ...
      const result = await paginatedFetch(
        opts,
        ({ limit, offset }) => searchSupplierCreditNotes(client, { filter: searchFilter, limit, offset, sort }),
        { label: 'Searching supplier credit notes', defaultLimit: 20 },
      );

      outputList(result as any, SCN_COLUMNS, opts as OutputOpts, 'Supplier Credit Notes');  // eslint-disable-line @typescript-eslint/no-explicit-any
    }));

  // ── clio supplier-credit-notes create ─────────────────────────
  scn
    .command('create')
    ...
    .action(apiAction(async (client, opts) => {
      ...
      if (body) {
        ...
        res = await createSupplierCreditNote(client, {
          ...body,
      ...
      if (opts.json) {
        ...
        console.log(chalk.green(`Supplier credit note created (${status}): ${res.data.reference || res.data.resourceId}`));
  ...
  scn
    ...
    .action((resourceId: string, opts) => apiAction(async (client) => {
      ...
      const res = await updateSupplierCreditNote(client, resourceId, data);

      if (opts.json) {
        console.log(JSON.stringify(res.data, null, 2));
      } else {
        console.log(chalk.green(`Supplier credit note updated: ${res.data.reference || res.data.resourceId}`));
      }
    })(opts));
  ...
  scn
    ...
    .description('Delete/void a supplier credit note')
    .option('--api-key <key>', 'API key (overrides stored/env)')
    .option('--json', 'Output as JSON')
    .action((resourceId: string, opts) => apiAction(async (client) => {
      await deleteSupplierCreditNote(client, resourceId);

      if (opts.json) {
        console.log(JSON.stringify({ deleted: true, resourceId }));
      } else {
        console.log(chalk.green(`Supplier credit note ${resourceId} deleted.`));
      }
    })(opts));
  ...
  scn
    ...
    .description('Record a refund against a supplier credit note')
    ...
    .action((resourceId: string, opts) => apiAction(async (client) => {
      ...
      if (body) {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any -- user-provided JSON, API validates
        res = await createSupplierCreditNoteRefund(client, resourceId, {
          ...body,
      ...
      if (opts.json) {
        ...
        console.log(chalk.green(`Refund recorded for supplier credit note ${resourceId}`));
      }
    })(opts));
  ...
  scn
    ...
    .description('List refunds for a supplier credit note')
    .option('--api-key <key>', 'API key (overrides stored/env)')
    .option('--json', 'Output as JSON')
    .action((resourceId: string, opts) => apiAction(async (client) => {
      const res = await listSupplierCreditNoteRefunds(client, resourceId);
      ...
      if (opts.json) {
        ...
        for (const r of refunds) {
          console.log(`  ${chalk.cyan(r.resourceId)}  ${r.reference || '(no ref)'}  ${r.status}  ${r.refundAmount}  ${normalizeDate(r.valueDate)}`);
        }
      }
    })(opts));

  // ── clio supplier-credit-notes draft ──────────────────────────
  const draft = scn
    .command('draft')
    .description('Manage draft supplier credit notes (inspect, finalize, attachments)');

  // ── clio supplier-credit-notes draft list ─────────────────────
  draft
    .command('list')
    .description('List all draft supplier credit notes with per-field validation status')
    ...
    .action(apiAction(async (client, opts) => {
      ...
      if (opts.ids) {
        ...
        items = await Promise.all(ids.map(async (id: string) => {
          const res = await getSupplierCreditNote(client, id);
          return res.data;
        }));
        ...
        const result = await paginatedFetch(
          ...
          ({ limit, offset }) => searchSupplierCreditNotes(client, {
            filter: { status: { eq: 'DRAFT' } },
            limit,
            offset,
            sort: { sortBy: ['valueDate'], order: 'DESC' },
          }),
          { label: 'Fetching draft supplier credit notes' },
        );
        items = result.data;
      }
      ...
      if (opts.json) {
        ...
        formatDraftTable('Supplier Credit Notes', reports);
      }
    }));

  // ── clio supplier-credit-notes draft finalize ─────────────────
  addDraftFinalizeOptions(
    draft
      .command('finalize <resourceId>')
      .description('Fill missing fields and convert a draft supplier credit note to APPROVED'),
  ).action((resourceId: string, opts) => apiAction(async (client) => {
    const body = readBodyInput(opts);
    ...
    if (cn.status !== 'DRAFT') {
      const msg = `Supplier credit note ${resourceId} is ${cn.status}, not DRAFT.`;
      if (opts.json) {
    ...
    // 7. FINALIZE
    await finalizeSupplierCreditNote(client, resourceId, updateData);

    // 8. VERIFY
    const verifyRes = await getSupplierCreditNote(client, resourceId);
  ...
  // ── clio supplier-credit-notes draft attachments ──────────────
  draft
    .command('attachments <resourceId>')
    .description('List attachments for a supplier credit note (URLs for agent inspection)')
    .option('--api-key <key>', 'API key (overrides stored/env)')
    .option('--json', 'Output as JSON')
    .action((resourceId: string, opts) => apiAction(async (client) => {
