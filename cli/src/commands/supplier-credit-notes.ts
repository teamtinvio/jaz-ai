  ...
  scn
    ...
    .action(apiAction(async (client, opts) => {
      ...
      if (opts.json) {
        ...
        console.log(chalk.bold(`Supplier Credit Notes (${result.data.length} of ${result.totalElements}):\n`));
    ...
    .action(apiAction(async (client, opts) => {
      ...
      if (opts.json) {
        ...
        if (result.data.length === 0) {
          console.log(chalk.yellow('No supplier credit notes found.'));
          return;
        }
        console.log(chalk.bold(`Found ${result.data.length} supplier credit note(s):\n`));
        const { items, overflow } = displaySlice(result.data);
        for (const cn of items) {
          const amount = cn.totalAmount !== undefined ? chalk.dim(` $${cn.totalAmount.toFixed(2)}`) : '';
    ...
    .action((resourceId: string, opts) => apiAction(async (client) => {
      ...
      if (opts.json) {
        ...
        if (attachments.length === 0) {
          console.log(chalk.yellow(`No attachments for supplier credit note ${cn.reference || resourceId}.`));
          return;
        }
