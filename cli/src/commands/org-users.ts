...
const ORG_USERS_COLUMNS: TableColumn[] = [
  ...
  { key: 'email', header: 'Email' },
  {
    ...
    format: (roles) => Array.isArray(roles)
      ? (roles as Array<{ moduleName: string; roleCode: string }>).map((r) => `${r.moduleName}:${r.roleCode}`).join(', ')
      : String(roles ?? ''),
  },
];
...
export function registerOrgUsersCommand(program: Command): void {
  ...
  ou
    ...
    .action(apiAction(async (client, opts) => {
      ...
      outputList(result as any, ORG_USERS_COLUMNS, opts as OutputOpts, 'Org Users');  // eslint-disable-line @typescript-eslint/no-explicit-any
    }));
  ...
  ou
    ...
    .action(apiAction(async (client, opts) => {
      ...
      if (opts.json) {
        ...
        console.log(success(`User invited: ${opts.firstName ?? ''} ${opts.lastName ?? ''} (${opts.email ?? ''})`));
  ...
  ou
    ...
    .action((resourceId: string, opts) => apiAction(async (client) => {
      ...
      if (opts.json) {
        ...
        console.log(success(`User updated: ${res.data.firstName} ${res.data.lastName}`));
      }
    })(opts));
  ...
  ou
    ...
    .action((resourceId: string, opts) => apiAction(async (client) => {
      ...
      if (opts.json) {
        ...
        console.log(success(`User ${resourceId} removed.`));
      }
    })(opts));
}
