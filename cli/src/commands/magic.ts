      ...
      if (opts.json) {
        ...
        if (validFile) {
          ...
          if (invalidFile?.errorMessage) {
            console.error(chalk.red(`Error: ${invalidFile.errorMessage}`));
          }
        }
      }
    }));
      ...
      if (opts.wait) {
        ...
        for (const id of ids) {
          ...
          if (res.data.length > 0) {
            ...
            if (!opts.json) {
              console.error(chalk.yellow(`  ${id}  NOT FOUND`));
            }
          }
        }
        outputWorkflowResults(results, opts.json);
    ...
    .action(apiAction(async (client, opts) => {
      ...
      if (opts.json) {
        ...
        for (const w of items) {
          ...
          console.log(`  ${chalk.cyan(w.resourceId)}  ${formatStatus(w.status)}  ${w.documentType}  ${w.fileName}${btInfo}  ${chalk.dim(w.createdAt)}`);
        }
...
function outputWorkflowResults(results: MagicWorkflowItem[], json?: boolean): void {
  if (json) {
    ...
    for (const w of results) {
      ...
      console.log(`  ${chalk.cyan(w.resourceId)}  ${formatStatus(status)}  ${w.documentType}  ${w.fileName}`);
      ...
      if (w.failureReason) {
        console.error(chalk.red('    Reason:'), w.failureReason);
      }
    }
  }
}
