...
if (shouldShowPicker() && process.argv.length === 2) {
  ...
  if (profileCount === 0) {
    ...
    process.stderr.write(chalk.dim('  No org connected.\n\n'));

    const { action } = await promptsPkg({
      type: 'select',
      name: 'action',
      message: 'Get started',
      choices: [
        { title: 'Connect an org (add API key)', value: 'add' },
        { title: 'Skip — use offline commands only', value: 'skip' },
      ],
    }, { onCancel: () => process.exit(0) });

    if (action === 'add') {
      const { key } = await promptsPkg({
        type: 'text',
        name: 'key',
        message: 'API key',
        validate: (v: string) => v.startsWith('jk-') ? true : 'API key must start with "jk-"',
      }, { onCancel: () => process.exit(0) });

      if (!key) process.exit(0);

      // Validate against API
      process.stderr.write(chalk.dim('  Validating...\n'));
      try {
        ...
        process.stderr.write(chalk.green(`  ✓ Connected: ${label} — ${org.name} (${org.currency})\n\n`));
        ...
        process.stderr.write(chalk.red('  ✗ API key is invalid or the API is unreachable.\n'));
        if (process.env.CLIO_DEBUG === '1') {
          process.stderr.write(chalk.dim(`  debug: ${(err as Error).message ?? err}\n`));
        }
...
if (shouldShowPicker() && notifier.update) {
  ...
  process.stderr.write(
    chalk.yellow(`  Update available: v${current} → v${latest}\n`),
  );
