...
| **MCP Server** | 241 tools your AI can call directly | AI agents and accountants that need live API access |
...
<details>
<summary>Available tool categories (241 tools)</summary>
...
With the MCP server running (`claude mcp add jaz -- npx jaz-clio mcp`), your agent has 241 tools for direct API access. Every tool returns structured JSON. See [tool categories](#option-4-mcp-server-for-ai-agents) for the full breakdown.
...
|-----------|-------|---------|
| `SKILL.md` | 370 | 117 rules — auth, IDs, dates, FX, payments, field aliases, response shapes |
| `endpoints.md` | 2104 | Request/response examples for every core endpoint |
...
| `full-api-surface.md` | 703 | Complete endpoint catalog (80+), enums, limits |
