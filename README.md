...
Pin `jaz-clio@5.24.12` for stability, or `jaz-clio@latest` for auto-updates. **Multi-org**: comma-separated keys, e.g. `"JAZ_API_KEY": "jk-aaa,jk-bbb"`. Personal access tokens (`pat-...`) also work for multi-org.
...
| What | How |
|------|-----|
| **MCP delivery** | 3 meta-tools (~600 tokens) instead of all 357 tools (~78KB). The agent searches into the catalog only when needed. |
