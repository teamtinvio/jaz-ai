...
Pin `jaz-clio@5.66.0` for stability, or `jaz-clio@latest` for auto-updates. **Multi-org**: OAuth supports accessible organizations with explicit `org_id` selection. Optional key-based access accepts comma-separated keys, e.g. `"JAZ_API_KEY": "jk-aaa,jk-bbb"`. Personal access tokens (`pat-...`) also work for multi-org.
...
{
  "command": "npx",
  "args": ["-y", "jaz-clio@5.66.0", "mcp", "--org", "oauth:<resourceId>"]
}
```
