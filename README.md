...
The complete agent surface for [Jaz](https://jaz.ai) accounting. 323 tools, 6 skills, 16 IFRS recipes, 13 calculators, 12 close playbooks. Works with any agent: Claude, GPT, Gemini, Copilot, Cursor. Token-lean discovery, one-shot tool selection, structured errors an agent can recover from.
...
Pin `jaz-clio@5.20.30` for stability, or `jaz-clio@latest` for auto-updates. **Multi-org**: comma-separated keys, e.g. `"JAZ_API_KEY": "jk-aaa,jk-bbb"`. Personal access tokens (`pat-...`) also work for multi-org.
...
- **323 tools** covering every Jaz endpoint. Each tool description disambiguates against similar tools, lists enum values inline, and flags idempotency. The LLM picks right on the first call.
...
| **CLI** (`jaz-clio`) | A `clio` binary: 63 command groups + 13 offline calculators + 12 offline blueprints + live API access. Humans run it; agents shell out to it. | You're scripting CI / running offline calculators / a human is at the terminal. |
| **MCP server** (`clio mcp`) | The same binary in MCP mode: 323 tools as agent-callable functions with structured envelopes. | This is the default for any agent (Claude / GPT / Gemini / Copilot / Cursor) that takes accounting actions. |
...
| What | How |
|------|-----|
| **MCP delivery** | 3 meta-tools (~600 tokens) instead of all 323 tools (~78KB). LLM searches into the catalog only when needed. |
...
- **CLI surface**: 63 command groups across the `clio` binary
