...
Plugins, skills, MCP server, and CLI for Claude Code, Cowork, Codex, Copilot, Cursor, and 30+ AI tools.
...
| **MCP Server** | 240 tools your AI can call directly | AI agents and accountants that need live API access |
...
The CLI gives you direct access to 48 command groups — financial calculators, job blueprints, API commands, and more — from your terminal.
...
The MCP server gives AI coding assistants direct access to 203 accounting tools — invoices, bills, journals, contacts, reports, financial calculators, job blueprints, and more. Runs locally on your machine.
...
<details>
<summary>Available tool categories (240 tools)</summary>
...
With the MCP server running (`claude mcp add jaz -- npx jaz-clio mcp`), your agent has 240 tools for direct API access. Every tool returns structured JSON. See [tool categories](#option-4-mcp-server-for-ai-agents) for the full breakdown.
...
|-----------|-------|---------|
| `SKILL.md` | 363 | 117 rules — auth, IDs, dates, FX, payments, field aliases, response shapes |
| `endpoints.md` | 2138 | Request/response examples for every core endpoint |
| `errors.md` | 860 | Error catalog with root causes and fixes |
| `field-map.md` | 618 | Intuitive name → actual field name mapping |
| `search-reference.md` | 724 | Filter fields, sort fields, operators for 28 search endpoints |
| `full-api-surface.md` | 702 | Complete endpoint catalog (80+), enums, limits |
...
```
src/
├── commands/                    48 command groups (invoices, bills, contacts, calc, jobs, mcp, ...)
├── core/
...
> **Tip:** Always pin `JAZ_API_KEY` in your MCP config rather than relying on the active profile. MCP servers cache credentials at startup — profile switches via `clio auth switch` won't take effect until the server restarts.
...
Alternatively, use `JAZ_API_KEY` in the MCP config's `env` block to pin a specific org — no restart needed when switching.
