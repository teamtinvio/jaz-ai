...
| **MCP Server** | 243 tools your AI can call directly | AI agents and accountants that need live API access |
...
<details>
<summary>Available tool categories (243 tools)</summary>
...
With the MCP server running (`claude mcp add jaz -- npx jaz-clio mcp`), your agent has 243 tools for direct API access. Every tool returns structured JSON. See [tool categories](#option-4-mcp-server-for-ai-agents) for the full breakdown.
...
|-----------|-------|---------|
| `SKILL.md` | 374 | 117 rules — auth, IDs, dates, FX, payments, field aliases, response shapes |
| `endpoints.md` | 2162 | Request/response examples for every core endpoint |
...
| `full-api-surface.md` | 705 | Complete endpoint catalog (80+), enums, limits |
