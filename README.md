...
<p align="center">
  ...
  <img src="https://img.shields.io/badge/API_rules-154-green?style=for-the-badge" alt="154 API Rules">
...
Pin `jaz-clio@5.5.1` for stability, or `jaz-clio@latest` for auto-updates. **Multi-org**: comma-separated keys, e.g. `"JAZ_API_KEY": "jk-aaa,jk-bbb"`. Personal access tokens (`pat_...`) also work for multi-org.
...
|-------|------------|-------------------|
| **Skills** | Domain knowledge as markdown (154 API rules, 16 recipes, 12 jobs, conversion playbooks, practitioner workflows). The agent reads these at session start. | Your AI tool reads markdown but cannot call binaries (e.g., a Custom GPT with no actions). |
...
|-----------|-------|---------|
| `SKILL.md` | 525 | 154 rules: auth, IDs, dates, FX, payments, field aliases, response shapes |
