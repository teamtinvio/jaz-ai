...
The complete agent surface for [Jaz](https://jaz.ai) accounting. 371 tools, 7 skills, 13 IFRS recipes, 13 calculators, 12 close playbooks. Works with any agent: Claude, GPT, Gemini, Copilot, Cursor. Token-lean discovery, first-try tool selection, structured errors an agent can recover from.
...
Pin `jaz-clio@5.62.0` for stability, or `jaz-clio@latest` for auto-updates. **Multi-org**: OAuth supports accessible organizations with explicit `org_id` selection. Optional key-based access accepts comma-separated keys, e.g. `"JAZ_API_KEY": "jk-aaa,jk-bbb"`. Personal access tokens (`pat-...`) also work for multi-org.
...
One catalog, three packagings. Every install reaches the **same 371 operations** — they are presented differently because hosts have different context budgets.
...
A directory listing that says "371 tools" and a client that shows 3 or 45 are describing the same server. Nothing is missing.
...
- **371 tools** covering every Jaz endpoint. Each tool description disambiguates against similar tools, lists enum values inline, and notes which operations are safe to retry. The model picks the right tool on the first call.
...
| **CLI** (`jaz-clio`) | A `clio` binary: 73 command groups + 13 offline calculators + 12 offline blueprints + live API access. Humans run it; agents shell out to it. | You're scripting CI / running offline calculators / a human is at the terminal. |
| **MCP server** (`clio mcp`) | The same binary in MCP mode: 371 tools as agent-callable functions with structured envelopes. | This is the default for any agent (Claude / GPT / Gemini / Copilot / Cursor) that takes accounting actions. |
...
- **CLI surface**: 73 command groups across the `clio` binary
...
{
  "command": "npx",
  "args": ["-y", "jaz-clio@5.62.0", "mcp", "--org", "oauth:<resourceId>"]
}
```
