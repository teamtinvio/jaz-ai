...
## [4.55.6] - 2026-04-12
- Automate MCPB GitHub releases: `publish-cli.yml` on the public mirror now builds the `.mcpb` from `cli/cli.mjs` and creates a versioned release on every push to main
- Anthropic's MCP directory pulls bundles from GitHub releases automatically — no more manual form/email uploads
- Fix `build-mcpb.sh` to produce versioned filename (`jaz-ai-<version>.mcpb`) matching the script's header comment
- Backfill CHANGELOG entry for 4.55.5
