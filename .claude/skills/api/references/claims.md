...
|--------|------|-----------|------|
| Create | `create_claim` | n/a | `valueDate` + `currencyCode` required; vendor = `contactResourceId` XOR `vendorName`; `saveAsDraft` defaults true (→ DRAFT), false validates + submits (→ SUBMITTED); **`reference` required when submitting** |
