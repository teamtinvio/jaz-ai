...
|-------|-----------|------------|-----------|
| `issueDate` | `valueDate` | `txnDateAliases` | POST/PUT: invoices, bills, customer-credit-notes, supplier-credit-notes, journals, cash-in-entries, cash-out-entries, cash-transfers, scheduled/invoices, scheduled/bills, scheduled/journals, scheduled/subscriptions |
...
| `taxRegistrationNumber` | `taxId` | UEN (SG), TIN (PH). API field is `taxId`. |
...
## Contacts (UPDATE/PUT request shape)
...
| `tag: "name"` (string) | `tags: ["name"]` (array) | ALL transaction types use `tags: string[]`. CLI wraps single `--tag` to array. |

**Supported on**: invoices, bills, customer credit notes, supplier credit notes, journals, cash-in, cash-out, cash transfers.
...
| What You'd Guess | Actual API Field | Notes |
|------------------|-------------------|-------|
| `customFields: { "PO Number": "PO-123" }` | `customFields: [{ customFieldName: "PO Number", actualValue: "PO-123" }]` | Array of objects, NOT key-value map |

**Supported on**: invoices, bills, customer credit notes, supplier credit notes (create + update).
**NOT supported on**: journals, cash entries, cash transfers.

### Nano Classifiers (classifierConfig)

| What You'd Guess | Actual API Field | Notes |
|------------------|-------------------|-------|
| `classifier: "Project Alpha"` | `classifierConfig: [{ resourceId, type, selectedClasses: [{ className, resourceId }], printable }]` | Array on each line item |
| `type` values | `"invoice"`, `"bill"` | Lowercase strings |

**Supported on**: line items of invoices, bills, credit notes; journal entries; cash entry details.
...
| `currency` / `exchangeRate` | DO NOT SEND | Currency and rates are derived server-side from bank account currencies |
